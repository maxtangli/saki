...
class RoundDrawTest extends PHPUnit_Framework_TestCase {
    function testNineKindsOfTerminalOrHonorDraw() {
        ...
        $this->assertEquals(RoundResultType::NINE_KINDS_OF_TERMINAL_OR_HONOR_DRAW, $r->getRoundResult()->getRoundResultType()->getValue());
    }
    ...
    function testFourWindDraw() {
        ...
        $r->debugDiscardByReplace($r->getCurrentPlayer(), $tileE);
        $r->passPublicPhase();
        $this->assertEquals(RoundResultType::FOUR_WIND_DRAW, $r->getRoundResult()->getRoundResultType()->getValue());
    }
    ...
    function testFourReachDraw() {
        ...
        $r->debugReachByReplace($r->getCurrentPlayer(), $tile, $tileList);
        $r->passPublicPhase();
        $this->assertEquals(RoundResultType::FOUR_REACH_DRAW, $r->getRoundResult()->getRoundResultType()->getValue());
    }
    ...
    function testFourKongDraw() { // todo handle kongPublicPhase
        ...
        $r->debugKongBySelfByReplace($r->getCurrentPlayer(), $tile);
        $this->assertEquals(RoundResultType::FOUR_KONG_DRAW, $r->getRoundResult()->getRoundResultType()->getValue());
    }
}