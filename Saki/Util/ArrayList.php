...
class ArrayList implements \IteratorAggregate, \Countable, \ArrayAccess {
    ...
    function getSum(callable $selector = null) {
        if ($selector === null) {
            ...
            $aggregator = function ($carry, $item) use ($selector) {
                return $carry + $selector($item);
            };
            return array_reduce($this->innerArray, $aggregator, 0);
        }
    }
    //endregion
    ...
    function shiftCyclicLeft(int $n) {
        ...
        $leftShiftCount = Utils::getNormalizedModValue($n, $this->count());
        if ($leftShiftCount == 0) {
            return $this;
        }
